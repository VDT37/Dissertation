...
- Margin against the codec floor: +0.0902 mm/h (positive = the learned mean beats the round-tripped advection field, i.e. mu_r learned something that survives to pixels).
- Margin against RAW advection: +0.0881 mm/h. This is the pixel-space headline, and it is a different question from the gate.
- Codec round-trip penalty on advection alone: +0.0021 mm/h.
- Fraction of the achievable codec span (floor to ceiling) recovered: 0.319.

## Pixel error (lower is better)

| metric | regression_mean | codec_advection | codec_oracle | advection | persistence |
|---|---|---|---|---|---|
| MAE (mm/h) | 0.2350 | 0.3252 | 0.0428 | 0.3231 | 0.3824 |
| RMSE (mm/h) | 0.9191 | 1.1425 | 0.1939 | 1.1404 | 1.2835 |
| bias (mm/h) | -0.1303 | -0.0139 | +0.0071 | -0.0191 | -0.0010 |

## CSI by threshold (higher is better)

| mm/h | regression_mean | codec_advection | codec_oracle | advection | persistence |
|---|---|---|---|---|---|
| 0.5 | 0.4277 | 0.4049 | 0.9171 | 0.4040 | 0.3074 |
| 1 | 0.3582 | 0.3226 | 0.8872 | 0.3212 | 0.2373 |
| 2 | 0.2552 | 0.2089 | 0.8379 | 0.2071 | 0.1525 |
| 4 | 0.1533 | 0.1069 | 0.7769 | 0.1054 | 0.0887 |
| 8 | 0.0713 | 0.0406 | 0.7377 | 0.0398 | 0.0485 |

## Detection at 1 mm/h

| metric | regression_mean | codec_advection | codec_oracle | advection | persistence |
|---|---|---|---|---|---|
| POD | 0.4186 | 0.4818 | 0.9510 | 0.4768 | 0.3827 |
| FAR | 0.2872 | 0.5060 | 0.0702 | 0.5039 | 0.6156 |
| freq_bias | 0.5872 | 0.9753 | 1.0228 | 0.9611 | 0.9957 |

## FSS (neighbourhood scales, km)

| field, threshold | 1 km | 5 km | 11 km | 21 km | 51 km | 101 km |
|---|---|---|---|---|---|---|
| regression_mean, 1 mm/h | 0.539 | 0.633 | 0.695 | 0.756 | 0.836 | 0.883 |
| regression_mean, 8 mm/h | 0.167 | 0.233 | 0.276 | 0.350 | 0.488 | 0.572 |
...
|---|---|---|---|---|---|---|---|
| regression_mean | 0.694 | 0.158 | 0.116 | 0.077 | 0.074 | 0.076 | 0.075 |
