...
def _ensure_vae_importable():
    ...
    if os.path.exists(os.path.join(here, "train_vae.py")):
        return
    for d in sorted(glob.glob(os.path.join(os.path.dirname(here), "*"))):
        if os.path.isdir(d) and os.path.exists(os.path.join(d, "train_vae.py")):
            sys.path.insert(0, d)
            return
_ensure_vae_importable()
from train_vae import VAE, AttnBlock, from_dbr, radial_psd, atomic_save, atomic_json
...
class NoiseEmb(nn.Module):
    ...
    def forward(self, t):
        ...
        return torch.cat([y.cos(), y.sin()], dim=1)
...
def sampled_diagnostics(denoiser, vae, diag, cond_diag, zA_diag, latent_scale,
    ...
    members = from_dbr(y_dbr).reshape(K, M, 256, 256)
    ...
    for k in range(K):
        ...
        crps_a += float(np.abs(A - y)[V].mean())
...
def load_diag_crops(latents_dir, va_ds, n_crops, leads=None):
    ...
    for i in diag_idx:
        row, s = va_ds[int(i)]
        f = index[s][int(va_ds.local[int(i)])]
...
def main():
    ...
    ap.add_argument("--sample-batch", type=int, default=128,
                    help="latents per sampling pass in the diagnostic "
                         "(crops x members)")
    ap.add_argument("--no-keep-sampled", dest="keep_sampled", action="store_false",
                    help="do not archive EMA weights as ckpt_epNNN.pt at each "
                         "sampled epoch (~263 MB each)")
    ap.add_argument("--psd-crops", type=int, default=64,
                    help="crops contributing to the diagnostic PSD ratio, capped "
                         "by --sample-crops")
    ...
    ap.add_argument("--leads", default=None,
                    help="comma-separated lead times (e.g. 15,30,45,60) for a "
                         "lead-conditioned model over the per-lead shards; omit "
                         "for a single-lead pack")
