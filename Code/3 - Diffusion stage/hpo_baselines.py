...
def _verify_psd_copy():
    ...
    for d in sorted(glob.glob(os.path.join(os.path.dirname(here), "*"))):
        if os.path.isdir(d) and os.path.exists(os.path.join(d, "train_vae.py")):
            ...
            break
    try:
        from train_vae import radial_psd as ref
    except Exception:
        return "not-checked (train_vae not importable here)"
    ...
    if not np.allclose(radial_psd(a), ref(a), rtol=1e-12, atol=0):
        raise SystemExit("ERROR: the local radial_psd copy has drifted from "
                         "train_vae.radial_psd. Fix the copy before scoring "
                         "anything: a PSD ratio computed under two definitions "
                         "is not comparable.")
    return "verified against train_vae.radial_psd"
...
def score(files, psd_crops, log):
    ...
    for _s, _l, f in files:
        ...
        P = np.nan_to_num(z["x_mmh"][-1].astype("float32"))
...
def main():
    ...
    ap.add_argument("--no-verify", action="store_true",
                    help="skip the check that the crop selection matches "
                         "train_ldm.LatentRows")
    ...
    if args.arm:
        ...
        if S.ARMS[args.arm]["kind"] == "infer":
            raise SystemExit("ERROR: the inference arm scores its baselines inside "
                             "evaluate.py, which already reports "
    ...
    for r in rungs:
        ...
        log("  the advection row is an integrity check: it must match the "
            "trainer's own mae_adv on these crops to within float noise")
