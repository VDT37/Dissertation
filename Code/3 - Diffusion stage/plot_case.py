...
from train_vae import from_dbr
...
def main():
    ...
    ap.add_argument("--arm", action="append", default=None, metavar="LABEL=CKPT",
                    help="a named model, e.g. --arm \"ml_v2=$ML/ckpt_ep025.pt\"; "
                         "repeat to put several models in one figure")
    ap.add_argument("--domain-json", default=None,
                    help="figures/uk_domain.json, supplying the projection and "
                         "grid origin for the basemap")
    ...
    ap.add_argument("--show-members", type=int, default=1, metavar="N",
                    help="individual ensemble members to draw per arm")
    ap.add_argument("--regression-mean", action="store_true",
                    help="draw the frozen stage-one conditional mean as a "
                         "reference panel; needs --mu-dir")
    ...
    if not sel:
        ...
        raise SystemExit(
            ...
            f"examples: {near or 'none'}\n  Use plot_domain.py --list-events to "
