...
def load_study(study_dir):
    ...
    for _c in _cands:
        ...
        if baselines:
            break
    return spec, ranking, plan, baselines
...
def leaderboard_rows(ranking, baselines):
    rows = []
    for r in ranking:
        m = r.get("metrics") or {}
...
def write_markdown(path, spec, rows, effects, trans, plan, baselines, top):
    ...
    a("Every trial below is scored against both baselines the brief requires, on "
      ...
      "extrapolation, computed once in `build_prior.py` and never "
      "learned.\n")
    if baselines:
        ...
        a("No baselines file was found, so the per-rung baseline table is "
          "omitted. The persistence and advection columns in the leaderboard "
          "come from each trial's own record and are unaffected. Run "
    ...
    if effects:
        ...
        if near_zero:
            a(f"{len(near_zero)} of {len(effects)} cells moved the objective by "
              f"less than 0.001, which is a measured null result rather than a "
              f"tuning win.\n")
    ...
    a("- A rung result is a ranking statistic, not a result. Nothing in this "
      ...
      "`evaluate.py`, and only those numbers go in the results "
      "chapter.")
...
def figures(study_dir, spec, rows, trans):
    try:
        import matplotlib
        matplotlib.use("Agg")
...
def main():
    ...
    if not args.no_figures:
        for p in figures(args.study, spec, rows, trans):
            print(f"wrote {p}")
