...
class VAE(nn.Module):
    def __init__(self, w=64, zc=4):
        ...
        self.enc = nn.Sequential(
            ...
            nn.Conv2d(w, 2 * w, 3, stride=2, padding=1), ResBlock(2 * w),
            nn.Conv2d(2 * w, 2 * w, 3, stride=2, padding=1), ResBlock(2 * w),
            nn.GroupNorm(8, 2 * w), nn.SiLU(),
            nn.Conv2d(2 * w, 2 * zc, 1),
        )
        self.dec = nn.Sequential(
            nn.Conv2d(zc, 2 * w, 3, padding=1), ResBlock(2 * w),
            nn.ConvTranspose2d(2 * w, 2 * w, 4, stride=2, padding=1), ResBlock(2 * w),
            nn.ConvTranspose2d(2 * w, w, 4, stride=2, padding=1), ResBlock(w),
