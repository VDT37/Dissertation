...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        B, C, Hh, Ww = x.shape
        h = self.norm(x)
        q = self.q(h).flatten(2).transpose(1, 2).unsqueeze(1)
...
def weighted_l1(recon, x, mask, mean, std, alpha, rcap):
    with torch.no_grad():
        R = torch.pow(10.0, (x * std + mean) / 10.0)
...
def main():
    ...
    ap.add_argument("--disc-start", type=int, default=-1,
                    help="global step at which the discriminator starts; "
                         "-1 = after 2 epochs")
    ...
    ap.add_argument("--psd-tol", type=float, default=0.15,
                    help="vae_best_psd.pt only considers epochs whose val rec_w "
                         "is within this fraction of the best")
    ...
    if resume_ck is not None:
        ...
        print("         Run pack_fields.py first for a ~5-10x faster epoch.", flush=True)
    ...
    if os.path.exists(bp):
        ...
        print(f"\ntotal time {(time.time()-t_all)/60:.1f} min | "
              ...
              f"latent_scale {ck['latent_scale']:.3f}", flush=True)
        if best_psd_ep:
            print(f"best PSD {best_psd_val:.3f} at ep{best_psd_ep} "
                  "-> vae_best_psd.pt", flush=True)
